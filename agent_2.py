...
class SnakeAI(nn.Module):
    ...
    def samplebatch(self):
        states = t.FloatTensor(self.memory_states)
        self.memory_states.clear()

        actions = t.IntTensor(self.memory_actions)
        self.memory_actions.clear()

        rewards = t.FloatTensor(self.memory_rewards)
        self.memory_rewards.clear()

        next_states = t.FloatTensor(self.memory_next_states)
        self.memory_next_states.clear()

        # breakpoint()
        isdones = t.IntTensor(self.memory_isdones)
        ...
        for i in range(episodes):
            ...
            for j in range(maxsteps):
                if (random.random() < self.epsilon):
                    ...
                    answer = self.forward(
                        (t.FloatTensor(state) * t.FloatTensor([5])))
                    print(answer)
    ...
    def train_snake(self):
        ...
        predict_target = rewards + 0.8 * predicted_future_value * isdones
        ...
        if (self.inp.weight.grad.norm() < 0.0001):
            self.inp.weight.grad.data += t.FloatTensor([0.001])
        self.optimizer.step()
        print(f"Ошибка: {loss}")
...
if __name__ == '__main__':
    ...
    snake = Snake()
    agent = SnakeAI(LearningRate=0.001, Epsilon=0.1)
    epoch = 1000
    epochs = list(range(epoch))
    for i in range(epoch):
        agent.game(snake, 1, 10000)
        agent.train_snake()
